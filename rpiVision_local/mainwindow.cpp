    ...
    if(videoCapture.isOpened()) {
       ...
       for(int i=0;i < (int) markers.size();i++) {
           ...
           if(markers[i].id == 0) {
			   ...
			   qDebug() << QString("Marker 0 Found. 0: ") + QString::number(origin[0]) + QString(", 1: ") + QString::number(origin[1]) + QString(", 2: ") + QString::number(origin[2]);
			   qDebug() << QString("origin x: ") + QString::number(originPx[0]) + QString("origin y: ") + QString::number(originPx[1]);
           }
			   ...
			   qDebug() << QString("Marker 1 Found. 0: ") + QString::number(xAxis[0]) + QString(", 1: ") + QString::number(xAxis[1]) + QString(", 2: ") + QString::number(xAxis[2]);
			   qDebug() << QString("xAxis x: ") + QString::number(xAxisPx[0]) + QString("xAxis y: ") + QString::number(xAxisPx[1]);
           }
			   ...
			   qDebug() << QString("Marker 2 Found. 0: ") + QString::number(yAxis[0]) + QString(", 1: ") + QString::number(yAxis[1]) + QString(", 2: ") + QString::number(yAxis[2]);
			   qDebug() << QString("yAxis x: ") + QString::number(yAxisPx[0]) + QString("yAxis y: ") + QString::number(yAxisPx[1]);
           }
           else {
...
Marker MainWindow::arucoMarkerToMarker(aruco::Marker marker)
{
    ...
    //Calculate x and y using pixels, not vectors
    float omPx[2];
    omPx[0] = marker[0].x - originPx[0];
    omPx[1] = marker[0].y - originPx[1];
    kmarker.pose.x = omPx[0]*oxPx[0] + omPx[1]*oxPx[1];
    kmarker.pose.x = kmarker.pose.x*arenaWidth/oxPx_mag;
    kmarker.pose.y = omPx[0]*oyPx[0] + omPx[1]*oyPx[1];
    kmarker.pose.y = kmarker.pose.y*arenaHeight/oyPx_mag;
...
void MainWindow::fillDefaultSettings()
{
    ...
    ui->frameRateLineEdit->setText("15");
    ...
    ui->cameraSharpnessSlider->setValue(245);
    ui->calibrationFileLineEdit->setText("/home/pi/Projects/visionSys/logitech_C920_1_calibration.yaml");
