...
{
    QByteArray data = serialPort->readAll();//serialPort->peek(1024);
    QList<QString> lines = (bufferString + QString(data)).split("*");
    QString toPrint;
    int numFullLines = (QString(data).endsWith("*")) ? lines.length() : lines.length() - 1;
    qDebug() << QString("DATA:\t") + QString(data);
    for (int x = 0; x < numFullLines; x++){
        if(!lines[x].contains("#") & (lines[x].length() > 0)){
            //Escape characters from message.
            toPrint = lines[x].remove(QRegExp("[^a-zA-Z0-9!?() .,_/\\\\\n\t+-]"));
            //Place received data in text edit.
            receivedDataTextEdit->insertPlainText(toPrint);
            qDebug() << QString("PRINT:\t")+toPrint;
        }
    }

    if(numFullLines < lines.length()){
        bufferString = lines[lines.length()-1];
    }else{
        bufferString = QString("");
    }
    qDebug() << QString("BUFFER:\t") + bufferString;
